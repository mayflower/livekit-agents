...
async def test_thinking_tokens_are_reported_and_counted_in_the_output(
    ...
    response_token_count, and total_token_count does not reliably cover both
    either. Reported as-is, every thinking token would be missing from the
    usage a cost view sums, so the billed output is understated by the whole
    reasoning budget.
    """
    async with _make_session(monkeypatch) as session:
        session._start_new_generation()
